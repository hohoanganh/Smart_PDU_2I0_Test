...
// ===== BASIC =====
void led_dbg_toggle() {
    digitalWrite(LED_DBG, !digitalRead(LED_DBG));
}
...
bool uart3_loopback() {
    ...
    while (SerialEXT.available()) SerialEXT.read();
    ...
    while (millis() - t0 < 100) {
        if (SerialEXT.available()) {
            if ((char)SerialEXT.read() != pat[i]) return false;
            if (++i == 6) return true;
        }
    }
    return false;
...
void flash_read_id() {
    ...
    SerialDBG.print(m, HEX); SerialDBG.print(" ");
    SerialDBG.print(t, HEX); SerialDBG.print(" ");
...
static void cli_execute(char *cmd) {
    ...
    if (strncmp(cmd, "rl ", 3) == 0) {
        ...
        if (sscanf(cmd, "rl %7s %7s", ch, st) == 2 &&
            ...
            if (strcmp(ch, "all") == 0) {
                for (uint8_t i = 1; i <= 4; i++) relay_set(i, on);
                ...
                if (n >= 1 && n <= 4) {
                    ...
                    if (ok) buzzer_beep(2000, 100);   // bip xac nhan dao relay
    ...
    // ----- LED1-4 (test truc tiep GPIO, doc lap voi relay) -----
        ...
        if (sscanf(cmd, "tled %7s %7s", ch, st) == 2 &&
            ...
            if (strcmp(ch, "all") == 0) {
                for (uint8_t i = 1; i <= 4; i++) led_set(i, on);
                SerialDBG.println("OK");
            } else {
                int n = atoi(ch);
                if (n >= 1 && n <= 4) {
    ...
    // ----- chung voi AK KIT -----
        ...
        SerialDBG.println(rs485_loopback() ? "RS485 LOOP OK"
                                           : "RS485 LOOP FAIL");
    }

    else if (strcmp(cmd, "u3") == 0) {
        SerialDBG.println(uart3_loopback() ? "UART3 LOOP OK"
                                           : "UART3 LOOP FAIL");
    }
        ...
        if (sscanf(cmd, "time %d:%d:%d", &hh, &mm, &ss) == 3 &&
            hh >= 0 && hh < 24 && mm >= 0 && mm < 60 && ss >= 0 && ss < 60) {
            SerialDBG.println(rtc_set_time(hh, mm, ss) ? "OK" : "RTC FAIL");
        } else {
            SerialDBG.println("Usage: time hh:mm:ss");
        }
    }

    else if (strcmp(cmd, "sht") == 0) {

        float t, h;

        if (sht45_read(&t, &h)) {
            SerialDBG.print("SHT45: ");
            SerialDBG.print(t, 1);
            SerialDBG.print(" C  ");
            SerialDBG.print(h, 1);
            SerialDBG.println(" %RH");
            SerialDBG.println("OK");
        } else {
            SerialDBG.println("SHT45 FAIL");
        }
    }
        ...
        if (sscanf(cmd, "baud %7s %lu", port, &b) == 2 &&
            ...
            if (strcmp(port, "rs485") == 0) {
                ...
                SerialDBG.println("ERR PORT (rs485|u3)");
            }
        } else {
            SerialDBG.println("Usage: baud rs485|u3 <1200-921600>");
        }
    }
        ...
        SerialDBG.println("flash");
        SerialDBG.println("fwr");
        SerialDBG.println("rs485 <text>");
        SerialDBG.println("rsl (loopback RS485)");
        SerialDBG.println("u3 (loopback PB10-PB11)");
        SerialDBG.println("rtc");
        SerialDBG.println("time hh:mm:ss");
        SerialDBG.println("sht");
        SerialDBG.println("baud rs485|u3 <bps>");
