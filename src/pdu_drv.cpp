...
#define RELAY_PULSE_MS 200 // xung kich relay chot 200ms roi nha (ca ON va OFF)

#define RELAY_NVM_ADDR 0x1000
#define RELAY_NVM_MAGIC 0xA5

static bool pca_present = false;

// relay_state khai bao truoc relay_save/load de tranh loi "not declared"
static bool relay_state[5] = {false};
static uint32_t relay_settle_until = 0;

static void relay_save() {
  uint8_t buf[5] = {RELAY_NVM_MAGIC, relay_state[1] ? 1 : 0,
                    relay_state[2] ? 1 : 0, relay_state[3] ? 1 : 0,
                    relay_state[4] ? 1 : 0};
  flash_write_bytes(RELAY_NVM_ADDR, buf, 5);
}

static void relay_load() {
  uint8_t buf[5] = {0};
  flash_read_bytes(RELAY_NVM_ADDR, buf, 5);
  if (buf[0] != RELAY_NVM_MAGIC)
    return;
  for (uint8_t i = 1; i <= 4; i++)
    relay_state[i] = (buf[i] == 1);
  SerialDBG.println("[Relay state restored from flash]");
}
...
static bool pca_init() {
  ...
  pca_present = pca_write(PCA_REG_OUT, 0x00) && pca_write(PCA_REG_CFG, 0x00);
  return pca_present;
}
...
// relay_state va relay_settle_until da khai bao o tren (truoc relay_save/load)
// =========================================================

// *** BANG MAP CHAN PCA -> RELAY ***
// RELAY_PIN[ch][0] = chan PCA khi OFF
// RELAY_PIN[ch][1] = chan PCA khi ON
// Chinh sua bang nay khi thay doi ket noi day
//
//  ch  |  OFF pin  |  ON pin
//  RL1 |    D2     |    D5
//  RL2 |    D3     |    D6
...
bool relay_set(uint8_t ch, bool on) {
  ...
  uint8_t pin = RELAY_PIN[ch][on ? 1 : 0];

  if (!pca_write(PCA_REG_OUT, (uint8_t)(1 << pin)))
    return false;
  ...
  while (millis() - _t0 < RELAY_PULSE_MS)
    buzzer_update();
  if (!pca_write(PCA_REG_OUT, 0x00))
    return false; // nha xung

  relay_state[ch] = on;
  led_update();
  relay_save(); // luu flash de khoi phuc sau reset

  if (ch == 4) {
    SerialDBG.println(on ? "RL_BATT: ON" : "RL_BATT: OFF");
  } else {
    SerialDBG.print("RL");
    SerialDBG.print(ch);
...
void relay_print_status() {
  for (uint8_t i = 1; i <= 4; i++) {
    if (i == 4) {
      SerialDBG.println(relay_state[i] ? "RL_BATT: ON" : "RL_BATT: OFF");
    } else {
      SerialDBG.print("RL");
      SerialDBG.print(i);
  ...
  if (!pca_present)
    SerialDBG.println("PCA9554: NOT FOUND");
}
...
void buzzer_beep(uint16_t freq, uint16_t time) {
  (void)freq;
  if (buzzer_muted)
    return; // dang tat tieng -> khong keu
  digitalWrite(BUZZER, BUZZER_ON_LEVEL);
  buzzer_off_at = millis() + time;
  if (buzzer_off_at == 0)
    buzzer_off_at = 1; // tranh trung gia tri 'tat'
}
...
// - Nha nut:              in "BTn: UP"
// (Quyet dinh dao relay o luc NHA nut: giu qua lau/nhieu nut se khong dieu
// khien)
// =========================================================
...
void btn_process() {
  ...
  if ((int32_t)(now - relay_settle_until) < 0)
    return;
  ...
  for (uint8_t b = 1; b <= 4; b++) {
    ...
    if (down && !pressed[b]) { // vua nhan xuong
      if (now - t_edge[b] < BTN_DEBOUNCE_MS)
        continue;
      pressed[b] = true;
      err_beeped[b] = false;
      hold_beeped[b] = false;
...
void drv_init() {
  ...
  digitalWrite(BUZZER, BUZZER_OFF_LEVEL); // tắt buzzer lúc khởi động

  for (uint8_t i = 1; i <= 4; i++) {
    pinMode(led_pins[i], OUTPUT);
    digitalWrite(led_pins[i], LED_OFF_LVL); // active-low: tắt = HIGH
  }
